...
function compileSCA(config, cb, generateTemplateManifests)
{
	...
	,	empty_tpl = 'empty_' + config.exportFile;
	...
	if (!useRequire)
	{
		var indexContent =	'require.config(' + JSON.stringify(config.amdConfig) + ');\n' +
							(!isSCIS ? 'var ' + config.entryPoint.replace(/\./g, '') + ' = ' : '') +   //var entrypoint used by fallback.io library to load dependencies
		...
		,	support_extensibility = (package_manager.distro.isSCA || package_manager.distro.isSCLite) && package_manager.isGulpLocal
		,	doUglify = (!args.nouglify && !package_manager.isGulpLocal)
		,	local_config = support_extensibility ? _.extend(config.amdConfig, { preserveComments:true, preserveFiles: false}) : config.amdConfig;
		;
		
		if(process.is_SCA_devTools)
		{
			shell.ShellString('define([]);').to(empty_tpl);
			
			config.amdConfig.loader = amdOptimize.loader(function(module_name)
			{
				if(/\.tpl$/i.test(module_name))
				{
				   return empty_tpl;
				}
		...
		stream = stream
			...
			.pipe(amdOptimize(indexModule, local_config))
			.on('error', package_manager.pipeErrorHandler)
			.pipe(gif(args.js === 'sourcemap' || support_extensibility, sourcemaps.init()))
			.pipe(map(function(file, cb)
			{
				if(process.is_SCA_devTools && path.basename(file.path) === empty_tpl)
				{
			...
			.on('error', package_manager.pipeErrorHandler)
			.pipe(gif(args.js === 'sourcemap' || support_extensibility, sourcemaps.write('.')));
	}
	else
	{
	...
	{
		shell.test('-f', empty_tpl) && shell.rm(empty_tpl);
		cb();
	});
}
...
function generateStarterLocalFiles(config)
{
	...
	var onEnd = function()
	{
		...
		var template = _.template('try {\nrequire.config(<%= require_config %>);\n var ' + entry_point.replace(/\./g,'') + '= require(<%= entry_point %>)\n} catch(e) { };');
...
gulp.task('libs', ['backward-compatibility-amd-unclean'], function(cb)
{
	...
	,	amdOptimizeConfig = {paths: {'Handlebars': 'handlebars.runtime-v4.0.10'}, preserveFiles:false, preserveComments:true};
