		...
		,	initialize: function(options) {
			...
			, childViews: _.extend({}, FacetsFacetedNavigationView.prototype.childViews, {
				'Facets.FacetedNavigationItems': function()
				{
					...
					_.each(this.options.appliedFacets, function(selected) 
					{
						if(selected.type == 2) {
							var name = selected.name;
							name = name.replace(/\s/g, "-");
							selected.nodeString = `${name}:${selected.value}`;
							selected_facets.push(selected);
						}
					});

					console.log("selected_facets item view", selected_facets);
